...
nls_m1d <- readRDS("13. RRR nls - negative exponential modelling/13. nls_FILE_CODE.rds")
...
check <- c("swet225")
spp <- c("Pinus echinata") 
idx_sub <- which(#nls_m1e$GRWRED30LWR_MEAN < 2 &
                  # nls_m1e$GRWRED50_MEAN < 2 &
                  #   abs(nls_m1e$MAX_RESIST - nls_m1e$MIN_RESIST) < 0.15 &
                   # nls_m1e$FILE_CODE %in% check
                   nls_m1e$SPECIES_ITRDB_NAME %in% spp &
                   nls_m1e$CLUSTER2 %in% 4
                 )
