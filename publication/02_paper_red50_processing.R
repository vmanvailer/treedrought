...
predicted_recovery[,CLUSTER2 := as.integer(CLUSTER2)]
predicted_recovery <- predicted_recovery |>
  merge(sp_list, by = "Id", all.x = TRUE) |>
  merge(cal_indices) |> 
  merge(unique(std_drought_clus[,.(CLUSTER2,COLOR)]), by = "CLUSTER2")
...
# Missing the following filter from script 18. Renumber clusters - Visualizing admin grouping world.R #83-84
  # ADMIN_GROUPING == "North America" & CLUSTER == 3 & LAT_DEC_DEG > 65 ~ NA,
  # ADMIN_GROUPING == "South America" & CLUSTER == 2 & LAT_DEC_DEG < -33 ~ NA,

# Ordering facets
facet_summ <- std_proj_recov_f[, .(mean_RED50Mean = mean(RED50Mean, na.rm = TRUE)), by = .(name, Species, COLOR)]
...
saveRDS(std_proj_recov_f, "publication/std_proj_recov_f.rds", compress = "xz")
saveRDS(ahm_mean, "publication/ahm_mean.rds", compress = "xz")
